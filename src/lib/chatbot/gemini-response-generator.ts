...
/**
 * Google Gemini Flash를 사용하여 자연스러운 대화 응답을 생성합니다.
 */
export async function generateGeminiResponse(
  ...
  try {
    ...
    // GoogleGenerativeAI 인스턴스를 함수 내부에서 생성
...
예시: "강릉의 바다 뷰 숙소 3곳을 추천드립니다! [PROPERTY_IDS: abc123, def456, ghi789]"`;
    ...
    // 5. Gemini 2.5 Flash 모델 초기화 (최신 빠른 모델)
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash",
    });

    // 6. 채팅 히스토리 변환 (system prompt를 첫 메시지로 포함)
    const chatHistory = [
      {
        role: "user",
      ...
      {
        role: "model",
        parts: [{ text: "네, 알겠습니다. VINTEE의 AI 상담 어시스턴트로서 등록된 숙소 정보를 바탕으로 정확하게 답변드리겠습니다." }],
      },
    ...
    // 10. [PROPERTY_IDS: ...] 부분 제거한 깔끔한 메시지 반환
    ...
    if (error instanceof Error && error.message.includes("API")) {
      return {
        message:
          "죄송합니다. AI 기능을 사용하려면 GEMINI_API_KEY 환경 변수를 확인해주세요.",
      };
    }
